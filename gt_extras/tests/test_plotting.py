...
def test_gt_plt_bar_bar_height_too_high(mini_gt):
    html = gt_plt_bar(gt=mini_gt, columns=["num"], bar_height=1234, height=567).as_raw_html()
    assert html.count('height="567"') == 6
    assert 'height="1234"' not in html

def test_gt_plt_bar_bar_height_too_low(mini_gt):
    html = gt_plt_bar(gt=mini_gt, columns=["num"], bar_height=-345, height=1234).as_raw_html()
...
@pytest.mark.xfail(reason="TypeError is expected for now, but should be fixed in the future")
